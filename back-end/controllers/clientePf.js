...
controller.novo = async (req, res) => {
   try {
      await ClientePf.create(req.body)
...
controller.listar = async (req, res) => {
   ...
   if(Object.keys(req.query).length > 0) {
      ...
      try {
         ...
         const lista = await ClientePf.find()
...
controller.obterUm = async (req, res) => {
   try {
      ...
      const obj = await ClientePf.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await ClientePf.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await ClientePf.findByIdAndDelete(id)
      if(obj) {
         res.status(204).end()
      }
      else {
...
async function busca (req, res) {
   ...
   try{
      let lista = await ClientePf.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
